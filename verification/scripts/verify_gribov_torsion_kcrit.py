...
def run_gribov_torsion_verification():
    ...
    print(f"\nHAUPTERGEBNIS:")
    print(f"k_stop = ET * 4*pi = {mp.nstr(ET, 5)} * {mp.nstr(4*pi, 15)}")
    ...
    print(f"g2_G = 4*pi/N_c = {mp.nstr(g2_Gribov, 15)}")
    print(f"alpha_s(Horizont) = 1/3 = {mp.nstr(alpha_s_G, 15)}")

    # Residual-Check: alpha_s = 1/3 exakt
    alpha_s_exact = mp.mpf('1') / mp.mpf('3')
    residual_alpha = abs(alpha_s_G - alpha_s_exact)
    print(f"Residual |alpha_s - 1/3| = {mp.nstr(residual_alpha, 5)}")
    ...
    print(f"\nSKALEN-HIERARCHIE:")
    print(f"Delta*        = {mp.nstr(Delta_star, 7)} MeV  [A]")
    print(f"k_geo     = {mp.nstr(k_geo, 10)} MeV  = Delta*/gamma  [A-]")
    ...
    print(f"k_stop / ET      = {mp.nstr(ratio_stop_ET, 20)} (soll = 4*pi)")
    print(f"4*pi               = {mp.nstr(4*pi, 20)}")

    # Residual k_stop/ET - 4*pi
    residual_4pi = abs(ratio_stop_ET - 4*pi)
    print(f"Residual |k_stop/ET - 4*pi| = {mp.nstr(residual_4pi, 5)}")
    if residual_4pi < mp.mpf('1e-14'):
        print("  PASS: k_stop = ET * 4*pi exakt (per Definition)")
    else:
    ...
    print(f"D_GZ(q) = q^2 / (q^4 + M_G^4 + Sigma_T * q^2)")
    print(f"Positivity verletzt für q < q_PV")

    # M_G = ET/sqrt(2) (Gribov-Masse bei Sigma_T = ET^2)
    M_G = ET / mp.sqrt(2)
    Sigma_T = ET**2  # UIDT-Identifikation
    print(f"M_G (Gribov-Masse) = ET/sqrt2 = {mp.nstr(M_G, 10)} MeV")
    print(f"Sigma_T = ET^2 = {mp.nstr(Sigma_T, 10)} MeV^2")
    ...
    if discriminant >= 0:
        ...
        if q_PV_sq > 0:
            ...
            print("q_PV^2 < 0: kein realer positivity-Bruch (erwartet für kleines M_G)")
    else:
    ...
    print(f"5kappa^2 = {mp.nstr(LHS, 20)}")
    print(f"3lambdaS = {mp.nstr(RHS, 20)}")
    print(f"|5kappa^2 - 3lambdaS| = {mp.nstr(residual_RG, 5)}")
    ...
    # ============================================================
    print(f"\nTORSIONS-KILL-SWITCH:")
    print(f"ET = {mp.nstr(ET, 5)} MeV != 0 -> SigmaT != 0 -> kein Kill-Switch-Aktivierung")
    print(f"k_stop(ET=0) = 0 * 4pi = 0 [Kill-Switch: kein Gribov-Stop ohne Torsion]")
    ...
    print("=" * 70)
    print(f"k_stop = ET * 4*pi = {mp.nstr(k_stop, 15)} MeV  [D]")
    ...
    print(f"Nächster Schritt: BRST-Kohümologie-Beweis für k_stop = ET*4*pi")
