...
--------------------------------------------------
b0 = 11*Nc / (3 * 16*pi^2)  [exact rational in mpmath]
beta_g = -b0 * g_s^3
...
- VacuumInstabilityException raised when |k^2*(1+u)| < 1e-70
...
class UIDT_FRG_ThreeCoupling_RK45:
    """Dormand-Prince RK45 solver for (kappa_FRG, lambda_S_run, g_s).
    ...
    def _beta_gs(self, g_s):
        ...
        Evidence: B (standard perturbative QCD, well-established)
        """
        mp.dps = 80
    ...
    # ------------------------------------------------------------------
    # Litim threshold integral  l_0(u) = 1/(1+u)^2
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Beta functions for scalar sector (LPA, d=4, O(1))
    # ------------------------------------------------------------------
    def _beta_scalar(self, kappa_frg, lam, u):
        """Returns (d_kappa_frg, d_lam, d_u).

        These are the standard O(N=1) LPA beta functions with Litim
        regulator in d=4.  The coupling g_s does NOT enter the scalar
        sector at this level (no scalar-gluon vertex in pure LPA).
        A g_s^2 threshold correction would require specifying the
        scalar-gluon vertex explicitly — that requires the resolved
        kappa_coup matching condition.  Until that is established the
        correction is set to zero and flagged as OPEN_MATCHING_PROBLEM.
        """
        mp.dps = 80
        l0  = self._litim_l0(u)
        pre = mpf('1') / (mpf('4') * pi**2)   # = 1/(4 pi^2), loop factor d=4
    ...
    # ------------------------------------------------------------------
    # Full RHS: state = [kappa_frg, lam, u, g_s]
    # ------------------------------------------------------------------
    ...
    def _dp_step(self, t, state, h):
        ...
        # Error estimate (requires k7 = f(t+h, y5))
        k7 = f(t + h, y5)
    ...
    def solve(self,
        ...
        ----------
        t_start_str   : RG time t = ln(k/Lambda_UV), typically -0.001
        t_end_str     : IR endpoint, typically -8 to -12
        kappa_frg_uv  : UV value of dimensionless field minimum
        lam_uv_str    : UV lambda_S; if None uses LEDGER value
        u_uv_str      : UV dimensionless mass squared u = m^2/k^2
        gs_uv_str     : UV strong coupling g_s (alpha_s = g_s^2/4pi ~ 0.3 at k~1 GeV)
        ...
        # Use ledger lambda_S if not overridden
        if lam_uv_str is None:
            lam_uv = LAMBDA_S_LEDGER
        else:
            lam_uv = mpf(lam_uv_str)

        kappa_coup = KAPPA_COUP_LEDGER

        # PRE-FLIGHT: RG constraint at UV
        self._check_rg_constraint(kappa_coup, lam_uv)

        t     = mpf(t_start_str)
        t_end = mpf(t_end_str)
        state = [mpf(kappa_frg_uv_str), lam_uv, mpf(u_uv_str), mpf(gs_uv_str)]
        h     = mpf('-0.05')   # UV -> IR: negative step
        ...
        while t > t_end:
            ...
            if err < self.tol:
                ...
                steps += 1
                # Adapt step size upward (never past 4x)
                if err > mpf('0'):
                    h_new = h * min(mpf('4'), mpf('0.9') * (self.tol / err) ** mpf('0.2'))
                    h = h_new
            else:
                # Reject step, shrink
                if err > mpf('0'):
            ...
            if fabs(h) < self.min_step:
                status = 'STIFF_HALT'
                k_crit = exp(t)
                print(f"[STIFF_HALT] Step size < min_step at t = {nstr(t, 10)}, k = {nstr(k_crit, 10)} GeV")
                break
        ...
        # Wilson-Fisher fixed point for O(1) LPA (analytic, d=4): kappa_frg* = 1/(6*lam*)
        wf_kappa_analytic = mpf('1') / (mpf('6') * lf) if lf != 0 else mpf('0')

        # TENSION ALERT: kappa_FRG vs kappa_coup
        ...
        report = {
            ...
            'OPEN_MATCHING_PROBLEM':
                ...
                'Evidence category for any shifted-FP claim: B (lattice-compatible) at best.',
        }
...
if __name__ == '__main__':
    ...
    solver = UIDT_FRG_ThreeCoupling_RK45(
        tol_str='1e-10',
        min_step_str='1e-20',
        Nc=3,
        Nf=0,
    ...
    #   kappa_FRG ~ 0.1  (near WF basin, d=4)
    #   lambda_S  = LEDGER (5*kappa_coup^2/3)
    #   u         = 0.01  (small positive mass, not tachyonic)
    #   g_s       = 1.2   (alpha_s ~ 0.11, near k ~ 1 GeV scale)
    history, report = solver.solve(
        t_start_str   = '-0.01',
        t_end_str     = '-8.0',
        kappa_frg_uv_str = '0.1',
        lam_uv_str    = None,     # use LEDGER
        u_uv_str      = '0.01',
        gs_uv_str     = '1.2',
    )
    ...
    for key, val in report.items():
        print(f"  {key:<30} : {val}")
    print("-" * 72)
    print(f"  Total steps accepted         : {report['steps']}")
