...
The dynamical vacuum structure of UIDT imposes a strict upper bound on the sum of neutrino masses to preserve stability against vacuum decay. As derived in Appendix~\ref{app:neutrino_mass}, the \textbf{Vacuum Stability Condition} requires:
\begin{equation}
    \sum m_\nu \le 0.16\,\text{eV}
\end{equation}
This theoretical prediction is in excellent agreement with the direct experimental limit from the KATRIN experiment ($m_{\nu_e} < 0.8$ eV) and tightens the cosmological bounds ($\sum m_\nu < 0.12-0.20$ eV) without invoking sterile states. For the full mathematical derivation of the "Trilateral Cosmological Convergence" (including $w_a \approx -1.300$ and $\sigma_8 \approx 0.79$), see \textbf{Appendix I}.
