...
\newcommand{\GammaInf}{\ensuremath{16.3437 \pm 0.0005}}          % [B]
\newcommand{\vVEV}{\ensuremath{47.7\MeV}}                        % [A]
\newcommand{\ETorsion}{\ensuremath{2.44\MeV}}                    % [C]
\newcommand{\LambdaGrid}{\ensuremath{0.66\,\mathrm{nm}}}         % [A]
\newcommand{\KappaRG}{\ensuremath{0.500 \pm 0.008}}              % [A]
\newcommand{\kappaVal}{\ensuremath{0.500}}                       % [A]
\newcommand{\LambdaS}{\ensuremath{\tfrac{5}{12}}}               % [A], RG fixed-point: 5kappa^2 = 3lambda_S
\newcommand{\wZero}{\ensuremath{-0.99}}                          % [C]
%===================================================================
...
{\small DOI: \href{https://doi.org/10.5281/zenodo.17835200}{DOI: 10.5281/zenodo.17835200}}\\[0.5em]
{\small 12~May~2026}\\[4em]
...
universe emerge as structural features of a manifold whose geometry is
...
has recently been termed \emph{structural actualism} (cf.\ Esfeld \& Deckert
2017~\cite{Esfeld2017})---an ontology where the identity of physical entities
...
the dimensionless couplings $\kappa$ and $\gamma$, with $\lambda_S = 5\kappa^2/3$ 
fixed by the RG constraint \catmark{A} (see Section~\ref{sec:massgap}). This is
...
Bare extrapolation: $\gamma_\infty = \GammaInf$ \catmark{B}.
\end{evidencebox}

\begin{remark}[The Dependent Coupling $\lambda_S$]
Within the \UIDT\ renormalisation flow, the auxiliary coupling $\lambda_S$ is not a free parameter. It is formally defined through the RG fixed-point identity $5\kappa^2 = 3\lambda_S$ (Category~A-). For a canonical $\kappa = \kappaVal$, this yields $\lambda_S = \LambdaS$, serving as a consistency constraint for the Banach contraction $q < 1$ in Theorem~\ref{thm:massgap}.
\end{remark}
...
a \textbf{~6.9$\sigma$ tension} (combined uncertainty) with the \UIDT\ canonical $H_0 = 70.4\kms$
...
propaging in a pre-existing (typically 10- or 11-dimensional) background. It
...
\begin{enumerate}[label=\textbf{L\arabic*:}, leftmargin=*]
  ...
  \item \textbf{Glueball $f_0(1710)$ Retraction (\catmark{E}).}
    ...
    $\Delta = 1.710 \pm 0.015\GeV$ with the physical mass of the $f_0(1710)$ scalar
...
$S(x)$ on the Rietz Grid with lattice spacing $\lambda = \LambdaGrid$.
...
Rietz Grid of spacing $\lambda = \LambdaGrid$ and torsion binding
...
\begin{enumerate}
  ...
  The lattice with spacing $\lambda = \LambdaGrid$ defines the minimal
...
\begin{itemize}
  ...
  $\lambda = \LambdaGrid$. This yields modified propagators and
...
\begin{enumerate}
  ...
  spacing $\lambda = \LambdaGrid$, including the back-reaction of
...
\emph{A Minimalist Ontology of the Natural World}
(Routledge, 2017).
DOI: \href{https://doi.org/10.1093/acprof:oso/9780198798217.001.0001}{10.1093/acprof:oso/9780198798217.001.0001}.
